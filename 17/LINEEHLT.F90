	PROGRAM LINEEHLT
	CHARACTER*1 S,S1,S2
	CHARACTER*16 FILEO,CDATE,CTIME
	COMMON /COM1/ENDA,A1,A2,A3,Z,C1,C2,C3,CW
	COMMON /COM2/T0,EDA0,AK0,AK1,AK2,CV,CV1,CV2,RO0,RO1,RO2,D0
	COMMON /COM3/E1,PH,B,U1,U2,R,CT/COM4/X0,XE/COM5/H2,P2,T2,ROM,HM,FM
	DATA PAI,Z,P0/3.14159265,0.68,1.96E8/,KT,S1,S2/0,1HY,1Hy/
	DATA N,X0,XE,W,E1,EDA0,R,Us,C1,C2,NZ,CU,CT/129,-4.,1.4,1.768E5,2.21E11,0.03,0.02,1.77,0.37,0.37,5,0.25,0.35/
	...
	ENDIF
	WRITE(*,*)'N,X0,XE,W,E,EDA0,R,US='
	READ(*,*)N,X0,XE,W,E1,EDA0,R,US
	IF(KT.EQ.2)THEN
	WRITE(*,*)'NZ,CU='
	READ(*,*)NZ,CU
	ENDIF
...
40  FORMAT(2X,'                     Wait     Please',//)
	...
	COMMON /COM1/ENDA,A1,A2,A3,Z,C1,C2,C3,CW
	COMMON /COMK/K/COMT/LT,T1(1100)/COM3/E1,PH,B,U1,U2,RR,CT/COM5/H2,P2,T2,RM,HM,FM
...
100 CONTINUE
	...
	IT=IT+1
	IF(KT.EQ.2.AND.IT.LT.10)GOTO 14
	KT=2
	IF(IT.GE.10)THEN
...
120 DO I=1,N
	...
	DIMENSION W(2200)
	COMMON /COM1/ENDA,A1,A2,A3,Z,C1,C2,C3,CW/COMK/K/COMT/LT,T1(1100)/COM2/T0,EDA0,AK0,AK1,AK2,CV,CV1,CV2,RO0,RO1,RO2,D0/COMAK/AK(0:1100)
...
4   W1=W1+P(I)
	...
	CALL VI(N,DX,P,W)
	GOTO 10
6   WX=-PAI1*W1*DX*ALOG(DX)
	DO 8 I=1,N
	W(I)=WX
	DO 8 J=1,N
	IJ=IABS(I-J)
8   W(I)=W(I)-PAI1*AK(IJ)*P(J)*DX
10  HMIN=1.E3
	DO 30 I=1,N
	H0=0.5*X(I)*X(I)+W(I)
	IF(KG.EQ.1)GOTO 20
...
60  H(I)=H00+H(I)+F0(I)
	...
	DO 100 I=1,N
	II=IT*(I-1)+1
	CT1=-0.05*T0*(T1(II)-1.0)
	CT2=D0*T0*(T1(II)-1.)
	EDA(I)=EXP(CT1)*EXP(A1*(-1.+(1.+A2*P(I))**Z))
...
100 CONTINUE
	...
	COMMON /COM1/ENDA,A1,A2,A3,Z,C1,C2,C3/COMAK/AK(0:1100)
	DATA PAI/3.14159265/
	DX1=1./DX
	DX2=DX*DX
	DX3=1./DX2
	...
	RI=-DX3*(D1*P(I-1)-(D1+D2)*P(I)+D2*P(I+1))+D4+R0(I)
	DLDP=-DX3*(D1+D2)+D6
	RI=C1*RI/DLDP
	GOTO 20
10  RI=-DX3*(D1*PP-(D1+D2)*P(I)+D2*P(I+1))+D4+R0(I)
...
30  IF(KG.EQ.2)GOTO 40
	R0(I)=-DX3*(D1*P(I-1)-(D1+D2)*P(I)+D2*P(I+1))+D4+R0(I)
	GOTO 70
40  R0(I)=DX3*(D1*P(I-1)-(D1+D2)*P(I)+D2*P(I+1))-D4+R0(I)
70  CONTINUE
	IF(KG.NE.0)GOTO 100
...
10  T0(K)=T(K)
	...
	COMMON /COM1/ENDA,A1,A2,A3,Z,O1,O2,O3/COM2/T0,EDA0,AK0,AK1,AK2,CV,CV1,CV2,RO0,RO1,RO2,D0/COM3/E1,PH,B,U1,U2,R,CC
	...
	B3=-0.05*T0
20  DO 30 K=1,NZ
	EDA(K)=EXP(A1*(-1.+B1))*EXP(B3*(T(K)-1.0))
...
60  B2=2.*CC2/(U1+U2)*(C1*(U1-U2)/EDA1(NZ)+C3*U1)
	...
	COMMON /COM2/T0,EDA0,AK0,AK1,AK2,CV,CV1,CV2,RO0,RO1,RO2,D0/COM3/E1,PH,B,U1,U2,R,CC
	DATA KK/0/
	IF(KK.NE.0)GOTO 20
	A1=U1
	A2=PH*(B/R)**3/E1
...
20  D(I)=B(1,I)*D(I-1)+B(2,I)
	...
	BLOCK DATA
	COMMON /COM2/T0,EDA0,AK0,AK1,AK2,CV,CV1,CV2,RO0,RO1,RO2,D0
	DATA T0,AK0,AK1,AK2,CV,CV1,CV2,RO0,RO1,RO2,D0/303.,0.14,46.,46.,2000.,470.,470.,890.,7850.,7850.,-0.00065/
	END